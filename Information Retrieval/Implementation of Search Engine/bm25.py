...
def get_index(term,dict_doc_score,doclen_dict,df_dict,qtf):
    ...
    try:         
        with open('documents/'+term+'.txt','r') as file1 :
            for line in file1.readlines() :
                ...
                K=float((0.3)+(0.9*(float(doclen_dict[docid])/52.83)))
                x=float(101*qtf[term]/(100+qtf[term]))
                y=float((2.2*tf)/(K+tf))
                bm_score = math.log((3204-df+0.5)/(df+0.5)) * x * y 
                
                
                try:
                    dict_doc_score[docid] = dict_doc_score[docid] + bm_score 
                except KeyError: 
                    dict_doc_score[docid] = bm_score
...
def get_score(query,doclen_dict,df_dict,qtf) :
    print "in get _score"
    wordList = [] 
    oktf_query_vector = []
    words = query.strip("\n").split(" ")
    ...
    for term in wordList:
       ...
       dict_doc_score = get_index(term, dict_doc_score,doclen_dict,df_dict,qtf) 
...
def main() :
    ...
    f1 = open(edited_queries, 'r') 
    f2 = open('result_processed_queries_fifth_model.txt', 'w')
    ...
    for query in f1.readlines(): 
        ...
        dict_id_score = {} 
        dict_id_score = get_score(query,doclen_dict,df_dict,qtf)
