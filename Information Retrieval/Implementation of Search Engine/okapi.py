...
def get_index(term, oktf_term, dict_doc_score,doclen_dict):
    try:         
        with open('documents/'+term+'.txt','r') as file1 :
            for line in file1.readlines() :
                ...
                oktf_for_doc = float(tf / (tf + 0.5 + 1.5 * float(doclen_dict[docid]) / 52.82))
                try:
...
def get_score(query,doclen_dict) :
    print "in get _score"
    wordList = [] 
    oktf_query_vector = []
    words = query.strip("\n").split(" ")
    ...
    for term in wordList:
       ...
       dict_doc_score = get_index(term, oktf, dict_doc_score,doclen_dict) 
...
def main() :
    ...
    f1 = open(edited_queries, 'r') 
    f2 = open('result_processed_queries_first_model.txt', 'w')
    ...
    for query in f1.readlines(): 
        ...
        rank = 1
        dict_id_score = {} 
        dict_id_score = get_score(query,doclen_dict)
