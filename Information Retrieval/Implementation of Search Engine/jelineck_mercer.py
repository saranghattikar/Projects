...
def get_index(term,dict_doc_score,doclen_dict,df_dict,qtf,unique_terms,docid_terms,total_words,ctf_dict):
    ...
    try:         
        with open('documents/'+term+'.txt','r') as file1 :
            for line in file1.readlines() :
                ...
                #p=math.log(float(0.2*(tf/float(doclen_dict[docid])+(0.8*(float(ctf_dict[term])/total_words))),10))
                p=math.log(float(0.2*(tf/float(doclen_dict[docid]))+(0.8*(ctf_dict[term]/total_words))),10)
                
                try:
                    dict_doc_score[docid] = dict_doc_score[docid] + p 
                except KeyError: 
                    dict_doc_score[docid] = p
...
def get_score(query,doclen_dict,df_dict,qtf,unique_terms,docid_terms,total_words,ctf_dict) :
    print "in get _score"
    wordList = [] 
    oktf_query_vector = []
    words = query.strip("\n").split(" ")
    ...
    for term in wordList:
       ...
       dict_doc_score = get_index(term, dict_doc_score,doclen_dict,df_dict,qtf,unique_terms,docid_terms,total_words,ctf_dict) 
...
def main() :
    ...
    f1 = open(edited_queries, 'r') 
    f2 = open('result_processed_queries_jelineck_mercer_model.txt', 'w')
    ...
    for query in f1.readlines(): 
        ...
        dict_id_score = {} 
        dict_id_score = get_score(query,doclen_dict,df_dict,qtf,unique_terms,docid_terms,total_words,ctf_dict)
        ...
        for eachdoc in docid_terms:
            for eachword in listofwords:
                if eachword not in docid_terms[eachdoc][1]:
                    try:
                        q=math.log((float(0.8*ctf_dict[eachword]/total_words)),10)
                        dict_id_score[eachdoc] = dict_id_score[eachdoc] + q
                    except KeyError :
                      continue 
