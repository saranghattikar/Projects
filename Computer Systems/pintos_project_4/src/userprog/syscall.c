...
static struct file_structure* get_file_structure(int fd) {
	...
	for (file_elem = list_begin(&current->opened_files);
			file_elem != list_end(&current->opened_files); file_elem =
					list_next(file_elem)) {
...
bool isdir(int fd) {
	...
	return file_struct->is_dir;
}

int inumber(int fd) {
	struct file_structure *file_struct = get_file_structure(fd);
	if (!file_struct) {
		PANIC("Could not create file_descriptor");
		return -1;
	}

	if (file_struct->is_dir) {
		return get_inumber(file_struct->dir ->inode);
	} else {
		return get_inumber(file_get_inode(file_struct->file));
	}
}
...
static int open_file(const char *file_name) {
	...
	if (is_inode_dir(file_get_inode(file))) {
		file_struct->is_dir = true;
		file_struct->file = NULL;
		file_struct->dir = (struct dir*) file;
	} else {
		file_struct->is_dir = false;
		file_struct->file = file;
		file_struct->dir = NULL;
	}
...
static void syscall_handler(struct intr_frame *f UNUSED) {
	...
	switch (*(int *) f->esp) {
		...
		case SYS_CREATE: {
			...
			f->eax = filesys_create((const char *) args[0], args[1], false);
			break;
		}
		case SYS_REMOVE: {
			extract_args(f, &args[0], 1);
