...
static void push_args (void **esp, char* file_name_args);
...
static void start_process (void *file_name)
{
  ...
  if (!success)
  {
    thread_current ()->is_loaded = THREAD_LOAD_NEGATIVE;
  } else
  {
    thread_current ()->is_loaded = THREAD_LOAD_POSITIVE;
  }
  // Signal that process is laoded successfully
  cond_signal (&thread_current ()->wait_for_me_to_load, &temp_lock);
...
void process_exit (void)
{
  ...
  /* Search itself in parent's children list and signal parent about its exit */
  if (cur->parent != NULL)
  {
        ...
        e != list_end (& ( parent->children )); e = list_next (e))
    {
      ...
      if (child->tid == cur->tid && child->tid != 2)
      {
        lock_acquire (&temp_lock);
...
bool load (const char *file_name_args, void (**eip) (void), void **esp)
{
  ...
  // Push args on stack
  push_args (esp, file_name_args_cpy);
 ...
 */
static void push_args (void **esp, char* commandline_args)
{
  int count = 0, argc = 0, word_align = 0;
  char *save_ptr, *token, *temp, *arg_align = 0;

  void *top_of_stack = *esp;
  void *bottom_of_stack = top_of_stack - PGSIZE;
  ...
  for (count = argc - 1; count >= 0; count--)
  {
    ...
    *esp = ( *esp - length );
    ...
    if(*esp < bottom_of_stack)
      exit_thread(-1);
    memcpy (*esp, argv [count], length);
  }
  ...
  for (count = argc - 1; count >= 0; count--)
  {
    *esp -= sizeof(char *);
    //check if esp is within the stack limit.
     if(*esp < bottom_of_stack)
       exit_thread(-1);
    memcpy (*esp, &address [count], sizeof(char *));
  }
